...
class YtdlpDownloaderApp:
    ...
    def setup_ui(self):
        ...
        clear_fmt_btn = tk.Button(
            ...
            command=lambda: (
                ...
                self.status_lbl.config(
                    text="Status: Format Cleared", fg=self.accent_purple
                ),
            ),
            bg=self.bg_surface,
            fg=self.fg_color,
        ...
        self.log_text.tag_config(
            "header", foreground="#89b4fa", font=("Consolas", 10, "bold")
        )  # Blue bold
    ...
    def set_buttons_idle(self):
        ...
        self.inspect_btn.config(
            state="normal", bg=self.accent_blue, fg=self.terminal_bg
        )
    ...
    def check_dependencies(self):
        ...
        if not os.path.exists(self.ytdlp_path):
            self.write_log(
                f"⚠️ Dependency Error: yt-dlp.exe was not found in: {self.script_dir}\n",
                False,
            )
            ...
            self.start_btn.config(
                state="disabled", bg=self.fg_disabled, fg=self.fg_color
            )
            self.inspect_btn.config(
                state="disabled", bg=self.fg_disabled, fg=self.fg_color
            )
    ...
    def browse_save_dir(self):
        ...
        selected_dir = filedialog.askdirectory(
            initialdir=current_dir, title="Select Download Directory"
        )
        if selected_dir:
    ...
    def on_mode_change(self):
        ...
        if mode == "clip":
            self.start_entry.config(
                state="normal", bg=self.bg_surface, fg=self.fg_color
            )
    ...
    def on_console_click(self, event):
        ...
        try:
            ...
            if not current:
                ...
                self.status_lbl.config(
                    text=f"Status: Selected Format {format_id}", fg=self.accent_green
                )
            else:
                parts = current.split("+")
                if format_id not in parts:
                    ...
                    self.status_lbl.config(
                        text=f"Status: Combined Formats {new_format}",
                        fg=self.accent_green,
                    )
                else:
                    ...
                    self.format_entry.insert(0, new_format)
                    self.status_lbl.config(
                        text=f"Status: Deselected Format {format_id}",
                        fg=self.accent_green,
                    )
        except Exception:
            pass
    ...
    def write_log(self, text, is_cr):
        ...
        if (
            clean_text.startswith("[info]")
            or "id  ext  resolution" in lower_text
            or clean_text.startswith("=" * 10)
        ):
            tags.append("header")
    ...
    def start_download(self):
        ...
        if not save_dir:
            messagebox.showerror(
                "Validation Error", "Please specify a download folder."
            )
            return
        ...
        if mode == "video":
            ...
            if not start or not end:
                messagebox.showerror(
                    "Validation Error", "Please specify both Start and End times."
                )
                return
            if not self.validate_time(start) or not self.validate_time(end):
                messagebox.showerror(
                    "Validation Error",
                    "Time arguments must be formatted as HH:MM:SS or MM:SS.",
                )
                return
        ...
        self.status_lbl.config(
            text="Status: Initializing Download...", fg=self.accent_purple
        )

        self.files_to_cleanup = set()
        self.current_save_dir = os.path.abspath(save_dir)

        # Start worker thread
        threading.Thread(
            target=self.run_command_thread, args=(cmd,), daemon=True
        ).start()
    ...
    def start_inspect(self):
        ...
        self.speed_lbl.config(text="")
        self.status_lbl.config(
            text="Status: Inspecting formats...", fg=self.accent_purple
        )
                ...
                if not os.path.exists(ffmpeg_path):
                    self.log_queue.put(
                        (
                            "log",
                            (
                                "⚠️ WARNING: 'ffmpeg.exe' was not found in the application folder.\n"
    ...
    def register_file_for_cleanup(self, filepath):
        """Resolves a filepath and registers it for deletion if the download is cancelled."""
        if not filepath:
            return
        if (
            not os.path.isabs(filepath)
            and hasattr(self, "current_save_dir")
            and self.current_save_dir
        ):
            filepath = os.path.join(self.current_save_dir, filepath)
        filepath = os.path.normpath(os.path.abspath(filepath))
        self.files_to_cleanup.add(filepath)
    ...
    def parse_and_log_line(self, line, is_cr):
        ...
        # Match destination filenames being written to register them for cleanup
        dest_match = re.search(r"Destination:\s+(.*)", line)
        if dest_match:
            filepath = dest_match.group(1).strip()
            filepath = filepath.strip("\"'")
            if filepath:
                self.register_file_for_cleanup(filepath)
    ...
    def cleanup_unfinished_files(self):
        """Deletes any unfinished files recorded during the aborted download process."""
        if not hasattr(self, "files_to_cleanup") or not self.files_to_cleanup:
            return
        ...
        # Try to clean up files with a short retry mechanism in case Windows OS locks take a split second to release
        for filepath in list(self.files_to_cleanup):
            possible_paths = [
                filepath,
                filepath + ".part",
                filepath + ".ytdl",
                ...
                if os.path.exists(p):
                    ...
                    for attempt in range(5):
                        try:
                            ...
                            time.sleep(0.1)  # Wait 100ms before retrying on lock
                        except Exception as e:
                            self.log_queue.put(
                                (
                                    "log",
                                    (
                                        f"⚠️ Failed to remove unfinished file '{p}': {str(e)}\n",
                                        False,
                                    ),
                                )
                            )
                    ...
                    if not deleted and os.path.exists(p):
                        self.log_queue.put(
                            (
                                "log",
                                (
                                    f"⚠️ Could not delete locked/unfinished file: {p}\n",
                                    False,
                                ),
                            )
                        )
    ...
    def stop_action(self):
        ...
        if self.current_process:
            try:
                ...
                # Clean up any unfinished files that were registered during this download run
                self.cleanup_unfinished_files()
            except Exception as e:
                self.log_queue.put(
                    ("log", (f"Error aborting execution: {str(e)}\n", False))
                )
    ...
    def on_process_finished(self, exit_code):
        ...
        if exit_code == 0:
            self.status_lbl.config(
                text="Status: Completed Successfully!", fg=self.accent_green
            )
            self.progress_var.set(100.0)
            messagebox.showinfo("Success", "The operation completed successfully!")
        elif (
            exit_code == -1 or exit_code == 128
        ):  # taskkill often leaves exit code 128 or -1
            ...
            self.status_lbl.config(
                text=f"Status: Failed (Code: {exit_code})", fg=self.accent_red
            )
            self.progress_var.set(0.0)
            messagebox.showerror(
                "Execution Error",
